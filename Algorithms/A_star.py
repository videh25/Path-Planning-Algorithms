"""@package A*
@brief Contains implementation class of A* Algorithm along with methods to display it on a map
...
class A_starAlgorithm(PathPlanningAlgorithm):
    """
    @breif Initialises A_star object
    ...
    def __init__(self, map: Map = None, map_path:str = None) -> None:
        ...
        if self.map.map_type == "Fine":
            print("[ERROR] A* algorithm can be applied to a Coarse Map only(with lesser number of nodes)")
            exit()
    ...
    def run(self, source_point: Tuple[int, int], target_point: Tuple[int, int], visual: bool = False) -> Tuple[int, List[int]]:
        ...
        if visual:
            ...
            self.map.show("A* Algorithm", live_fig, live_ax, False)
        ...
        check_queue = []
        # Priority Queue Entries: (combined_metric, node_number, via_node, distance)
        for node in range(len(node_array)):
            if node == source_node:
                hq.heappush(check_queue,[0, node, -1, 0])
                continue
            hq.heappush(check_queue,[np.inf, node, -1, np.inf])
        ...
        while just_poped != target_node:
            ...
            _, just_poped, jp_via, jp_distance = hq.heappop(check_queue)
            ...
            for node, cost in enumerate(cost_matrix[just_poped]):
                ...
                if check_queue[index][3] >= jp_distance + cost:
                    check_queue[index][3] = jp_distance + cost
                    check_queue[index][0] = check_queue[index][3] + np.linalg.norm(np.array(node_array[node]) - np.array(target_point))
...
if __name__ == "__main__":
    PPA = A_starAlgorithm(map_path='Maps/demo_maps/30x10_B.png')
    PPA.visualise_graph()
    PPA.operate(True)